...
#define TESTMAXSIZE 100000
#define LOGPOINT 10000
...
int main(int argc, char *argv[])
{
    ...
    //std::map<int, Elem *> mData;
    std::map<TKey, Elem *> mData;
    unsigned long nextLogPoint = LOGPOINT;
    ...
    if (fromFile) {
        ...
        std::string testCase;
#endif
        for (unsigned long i=0; i<TESTMAXSIZE; ++i){
            int r = rand();
#ifdef TEST_CASE_FILE
...
//    }
    ...
    //НАдо сделать так чтобы тест целиком прогонялся на контейнерах разного объёма
     //       т.е. тест на 1000, 1000*10, *10 = Тогда O(n) получится
    ...
    //mFun.insert(std::make_pair(std::string("SpeChache"),testSpeChache));
    //mFun.insert(std::make_pair(std::string("OChache"),testOChache));
    ...
    for (auto p: mFun){
        ...
        strFind.append(p.first)
                .append(".find (micros)");
        unsigned int testSize = 0;//LOGPOINT;
        unsigned int capacity = 0;//testSize/10;
        p.second(mData, strInsert, strFind, 10000, 1000);
        p.second(mData, strInsert, strFind, 100000, 10000);
        //p.second(mData, strInsert, strFind, 10000, 1000);
//        while(testSize<TESTMAXSIZE){
//            testSize+=LOGPOINT;
//            capacity=testSize;// /10;
