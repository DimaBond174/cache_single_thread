...
class  TONode2  {
 public:
  ...
  unsigned int  used_count;  // is it useful? need it in cache?
};  //  TONode2
...
class  OnCache2  {
 public:
    ...
    */    
  OnCache2(unsigned long long  capacity,  unsigned int  uselessness,
           TDeletDataFunc f_delData)
      :   l_f_delData(f_delData),  l_hash_baskets(capacity), l_uselessness(uselessness) {
    init();
  }
  ...
  void  insertNode  (TKey const  *key,  const char  *data)  {
    ...
    const unsigned long long  basketID  =  hash % l_hash_baskets;
...
//  void  find_bug()  {
//    for (unsigned long long b  =  0;  b  <  l_hash_baskets;   ++b)  {
...
//    }
//  }  // find_bug
//  unsigned long long  getRateCount()  {
...
//  }
 ...
 private:
  //const unsigned int  _capacity; == _hash_baskets
  const unsigned long long  l_hash_baskets;
  ...
  unsigned long long  leafAllocCounter  =  0;
...
const unsigned int  l_uselessness;
unsigned long long  rating_len  =  1;

    //Landscapes    
//  unsigned char  landscape_h[256];
//  unsigned char  *land_h_p;
  unsigned char  landscape_l[256];
  unsigned char  land_l_p  =  0;
...
//    headNode.leastUseful  =  &headNode;
  ...
  void  clear()  {
    for (unsigned long long  i  =  0;  i  <  leafAllocCounter;  ++i) {
      if (curLeaf[i].data)  {
  ...
  void  allocNode(const unsigned long long  hash,  TKey const  *key,
      const char  *data,  const unsigned long long  basketID,  int  cmp)  {
    TONode2  *re  =  nullptr;
    ...
    if (!re)  {
      ...
      if (hash  ==  re->hash)  {
        ...
        // Other hash case
        const unsigned long long  re_basketID  =  re->hash % l_hash_baskets;
        delWithOtherHash(re,
   ...
   */
    int  setll(unsigned long long  hash,
        TKey const  *key,  const unsigned long long basketID)  {
      TONode2  *cur  =  &(baskets[basketID]);
      hashOut  =  hash;      
      updatePathOutH  =  cur;
